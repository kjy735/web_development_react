...
터미널 상에서의 경로
 ...
 2. tsconfig.json 파일은 컴파일된 자바스크립트 출력의 대상 버전이나 이용되는 모듈 시스템과 같은 컴퍼일러 옵션을 지정하기 위해 타입스크립트에서 이용하는 구성파일. vite에서 정의한 기본 설정을 이용 가능
...
그런데 이상의 경우 event 부분에 빨간줄이 있음을 확인
'event' 매개변수에는 암시적으로 'any' 형식이 포함됨
(parameter) event: any

라고 작성돼있는데, 이것은 정적 타입 분석기인 typescript기준으로 자료형이 명시되어 있지 않기때문에 나오는 경고입니다
...
이상과 같이 자료형을 명시했는데 input에 onChange가 적용되는 부분과, form 에서의 onSubmit에 들어가는 매개변수 상의 차이점입니다.

이부분은 처음 언급하는 내용이니 확인하시면 되겠습니다.

## vite와 typescipt
vite는 타입스크립트 파일을 자바스크립트 파일로 변환하기는 하는데 타입 검사를 수행하지 않습니다. 비트는 타입스크립트 컴파일러(tsx)보다 빌드 속도가 빠른 esbuild를 이용하여 타입스크립트 파일을 트랜스파일합니다.
...
: 비동기 연산을 처리하는 전통적인 방법으로는 연산의 성공 / 실패에 콜백 함수를 적용하는 형태입니다. 성공하면 success 함수가 호출, 실패하면 failure함수가 호출되는 방식이었습니다.
...
이상의 예제가 콜백 함수 사용법을 기본 개념에 해당하는데, 

오늘날 프로미스는 JS에서 비동기 프로그래밍의 기본 요소입니다. 프로미스는 비동기 연산의 결과를 나타내는 '객체'에 해당. 이용하면 비동기 호출을 실행할 때 코드가 단순화. 프로미스 또한 비차단(non--blocking) 방식으로, 프로미스를 지원하지 않는 구형 라이브러리를 비동기 연산에 이용하는 경우 코드를 읽고 유지 관리하는 것이 어렵고, 이 경우 읽기 어려운 여러 갱의 중첩된 콜백이 생기게 됩니다. 각 콜백에서 오류를 확인해야 하므로 오류 처리도 어렵습니다.
...
doAsynsCall().then(response => {})  // 응답된 결과는 argument로 가지고 추가 연산 및 작업을 수행
```

- then() 메서드의 리턴값 -> Promise 객체인데, 세 가지 상태 중 하나에 속합니다.
1. 대기 : 초기상태
2. 이행(fullfilled / resolved) : 작업성공
3. 거부(rejected) : 작업실패
...
```
이상의 코드는 간단한 프로미스 '객체'로, setTimout은 비동기 연산을 시뮬레이션하는 역할로 브라우저 자체 함수입니다(두 번재 argument로 밀리초 개념을 받습니다. 여기서 두 번째 argument 빼놓고 작성하시는 분들이 많습니다).

코드 풀이 :

객체가 생성될 때와 타이머가 실행되는 동안에 프로미스는 대기 상태에 빠지게 됩니다. 500밀리초가 지나게 되면 Hello라는 argument를 가지는 resolve 함수가 호출되고, '이행' 상태가 됩니다. 오류가 발생하면 프로미스 상태가 '거부'로 변경되며 이 오류는 추후 catch() 함수를 통해 처리하게 됩니다.
...
catch()을 이용하여 프로미스에 오류 처리를 추가하는 것도 가능. catch()는 then 체인에서 오류발생하면 실행됩니다.
...
: 비동기 호출을 처리하는 더 최신 방법은 ES2027에 도입된 async / await을 이용하는 방법 -> 프로미스를 기반으로 하는데 await을 포함하는 async() 함수를 정의 해야 합니다.
...
- fetch() 개념이 좀 중요한데 -> 프로미스를 반환합니다. -> 근데 then안쓰고 await 씁니다
...
- 이상의 코드는 async / await과 함께 try...catch를 통한 예외처리 구문이었습니다.
...
fetch AIP를 이용하면 WEB 요정이 가능. fetch API의 개념 자체는 기존의 XMHttpRequest 또는 jQuery Ajax API와 유사한데, fetch API는 프로미스 또한 지원하므로 이용하기가 더 편리합니다. fetch를 이용하는 경우 라이브러리를 설치할 필요 없이 최신 브라우저에서 기본적으로 지원합니다.(저희는 크롬을 씁니다.)
...
.catch(err => console.log(err)) // 에러가 있다면 에러를 출력
```

fetch() 메서드는 응답(reaponse)이 포함된 프로미스를 반환. .json() 메서드를 통해 응답 결과를 json 데이터를 추출할 수 있으며, 이 메서드 역시 프로미스를 반환(그러니까 두번째 .then()을 실행시킬 수 있음)

```js
fetch('http://someapi.com')
.then(response => {
  if(response.ok) {
    // 요청 성공 -> 상태는 2xx (200: ok / 201: created )
  }
})
...
- 두 번째 argument에 헤더를 추가하는 것도 가능(springboot)
...
3. 요청 본문 내에 JSON으로 인코딩된 데이터를 보내는 경우 예시
