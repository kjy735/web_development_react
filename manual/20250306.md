...
App.jsx에 전부 다 지우고 return에 <></>만 남김
...
descriptions, icon, temp의 정보를 가지고 오기 위한 주소지를 찾으시오.
...
이라고 가정했을 경어, 가격 1.8을 출력하기 위해서는
console.log(MENU["에스프레소"]["가격"])이라고 주소지를 성정해야 함.
...
react dev-tools 상에서 App을 클릭하니까 temp / desc / icon 에 값이 들어가있는 것을 확인할 수 있었습니다.
temp / desc / icon 순으로 했었지만 그 순서가 아니라
알파벳 순으로 정렬됐던 것을 확인할 수 있는데
객체의 key가 입력순서를 담보하지 않기 때문입니다.

개발자 도구 상에서 Network 탭에서 200 ok인 것도 확인

이상까지 확인했을 때 데이터를 가지고 온 것까지 완료했지만
렌더링 되는 부분을 작성하지 않았습니다 그래서 return 문을 작성할텐데
...
해당 파트에서 낯선 부분이라면 fetch와 관련되어있는 부분인데, 추후 더 설명할 예정이지만, 기본적 개념은 20250305.md 파일에 ctrl + f 눌러서 fetch 검색하기 바랍니다.
...
```tsx
// function App 빼놓고 복사해옴
  const [keyword, setKeyword] = useState('');
  const [repodata, setRepodata] = useState<Repository[]>([]); // Repository 타입의 배열
```

사용자 입력의 키워드를 이용하여 REST API 호출을 만들 예정. 입력 필드와 버튼을 구현해서 적용
...
- 이상까지 입력한 이후 return 문에서 입력 필드와 버튼을 구현하고, 입력 필드에 onChange 관련 리스너 를 추가하여 입력값을 keyword 상태에 저장할 수 있도록 해야합니다. 버튼에는 클릭 리스너가 있고, 주어진 키워드로 REST API를 호출하도록 함수를 작성할겁니다.

- return 완성 후에 저희는 handleClick 로직을 작성할건데, 백틱을 이용하여 url과 keyword 상태를 연결시킬겁니다. 요청을 전송하기 위해서 axios.get() 메서드 활용할건데 fetch와 다르게 .json() 사용할 필요가 없다는 점도 기억해야 할 부분이겠습니다.

Axios는 자동으로 응답 데이터를 파싱(parsing)한 다음 응답 데이터와 items 배열을 repodata 상태에 저장할겁니다. 그리고 catch를 사용하여 오류를 처리할겁니다.
...
이상까지 코드를 작성하게 되면 repodata 상태에 값이 저장됨. 이상의 값을 브라우저에서 볼 수 있도록 return을 수정해줘야함

map()을 이용하여 data 상태를 테이블 행으로 변환시킬 예정
...
-> 그래서 왠만하면 개인 프로젝트를 placeholder로 걸기 바랍니다...

-> 다만 repository를 검색하는거기 때문에 github id 기준이 아닙니다.
아이디 뒷 부분의 / 이후를 확인합니다.

* 참고 : github API는 요청 횟수 제한이 적용돼서 너무 많이 써먹으면 안됩니다. 1분에 10회 요청제한이 있어서 그 이상 검색하고 나면 1분 기다려야 합니다.

### 경쟁 조건 처리

: 컴포넌트가 여러 요청을 빠르게 수행하면 예측할 수 없거나 잘못된 결과를 생성할 수 있는 경쟁조건(race condition)이 발생할 수도 있습니다. 네트워크 요청은 비동기적이므로 요청이 전송된 순서대로 완료되지 않을 수도 있습니다.
